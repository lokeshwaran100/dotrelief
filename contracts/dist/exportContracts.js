...
let dirEntries = [];
try {
    dirEntries.push(...fs.readdirSync(path.join(".deploys", "pinned-contracts"), { recursive: true, withFileTypes: true }));
    dirEntries.push(...fs.readdirSync(path.join(".deploys", "deployed-contracts"), { recursive: true, withFileTypes: true }));
}
catch (e) {
...
dirEntries = dirEntries.filter((entry) => entry.isFile() && entry.name.startsWith("0x") && entry.name.endsWith(".json"));
