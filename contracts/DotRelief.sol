...
contract DotRelief {
    ...
    // Modifier to check if the sender is member of DotRelief project
    modifier onlyMember() {
        require(isDaoMember(msg.sender), "Not a member of DotRelief project");
        _;
    }

    // Function to check if the given member is part of DotRelief project
    ...
    // Function to add member to the DotRelief project
    function addMember(address member) public onlyMember {
        require(!isDaoMember(member), "The member is already part of DotRelief project");
        members.push(member);
        emit MemberAdded(member);
    }

    // Function to remove member from the DotRelief project
    function removeMember(address member) public onlyMember {
        require(!isDaoMember(member), "Not a member of DotRelief project");
    ...
    function createDirectProposal(string memory _name, uint256 _amount) public returns(uint256) {
        require(_amount * 1 ether > address(this).balance, 
                "DotRelief project has insufficient funds.");
